...
while app is True:
    num_jugadores = ''
    funciones.crear_jugadores(num_jugadores, lista_jugadores, dict_jugadores)
    funciones.decidir_orden(mazo, lista_jugadores, dict_jugadores, orden_jugadores, orden_inicial)
    ...
    while game is True:
        ...
        for jugador in orden_jugadores[1::]:
            ...
            if turno == 1:  # El primer turno la apuesta es fija (20% de total)
                ...
                while int(dict_jugadores[jugador]["apuesta"]) > dict_jugadores[jugador]["puntos_restantes"]:
                    dict_jugadores[jugador]["apuesta"] = input(f'Apuesta incorrecta. No puede ser superior a tus puntos restantes, entonces...\n'
            ...
            while accion != '1' or accion != '2':
                ...
                if accion == '1':
                    ...
                    for carta in dict_jugadores[orden_jugadores[0]]['cartas']:
                        print(f'{carta[0]} de {carta[1]}')
                    print(f'\n({orden_jugadores[0]}) tiene {dict_jugadores[orden_jugadores[0]]["puntos_mano"]} puntos.')
        ...
        for jugador in orden_jugadores[1::]:
            if (dict_jugadores[orden_jugadores[0]]['puntos_mano'] >= dict_jugadores[jugador]['puntos_mano'] and
                dict_jugadores[orden_jugadores[0]]['mano'] != 'eliminado') or (
                    dict_jugadores[jugador]['mano'] == 'eliminado' and banca_gana_automaticamente is False):
                dict_jugadores[orden_jugadores[0]]["puntos_restantes"] += dict_jugadores[jugador]["apuesta"]
                dict_jugadores[jugador]["puntos_restantes"] -= dict_jugadores[jugador]["apuesta"]
        ...
        for jugador in orden_jugadores[1::]:
            if (dict_jugadores[orden_jugadores[0]]['puntos_mano'] < dict_jugadores[jugador]['puntos_mano'] or
                ...
                if dict_jugadores[jugador]['puntos_mano'] == 7.5:
                    if (dict_jugadores[jugador]["apuesta"] * 2) > dict_jugadores[orden_jugadores[0]]["puntos_restantes"]:
                ...
                for jugador in orden_jugadores[1::]:
                    if dict_jugadores[jugador]['mano'] == 'eliminado':
                        ...
                        if dict_jugadores[orden_jugadores[0]]['puntos_mano'] >= dict_jugadores[jugador]['puntos_mano']:
                            print(f'{jugador} pierde {dict_jugadores[jugador]["apuesta"]} puntos.')
                        elif dict_jugadores[orden_jugadores[0]]['puntos_mano'] < dict_jugadores[jugador]['puntos_mano']:
                            if dict_jugadores[jugador]['puntos_mano'] == 7.5:
                                print(
                                    f'{jugador} ha sacado 7 y Medio, gana {dict_jugadores[jugador]["apuesta"] * 2} puntos de la banca')
                            elif dict_jugadores[jugador]['puntos_mano'] != 7.5:
        ...
        if len(orden_jugadores) == 1:
            ...
            game = False
        if turno == 30:
            print(f'Ha terminado la partida. Los jugadores se quedan con las siguietes puntuaciones:\n')
            for jugador in orden_jugadores:
...
'''
    En esta version del bucle.
        1) Hemos empezado a encapsular código en funciones en el archivo funciones.py
            crear_mazo / crear_jugadores / decidir_orden / repartir_cartas
        2) Arreglado error en comprobacion de apuesta si el input era ''
        *) Impresion del informe de final de partida en turno 30 ( HAY QUE MEJORAR CON UN BUBBLE SORT SEGUN PUNTUACIONES) - Pendiente
